...
class Sentinel
{
    ...
    public function registerAndActivate(array $credentials)
    {
        ...
        return $this->register($credentials, $callback);
    }
    ...
    /**
     * Inactivates the given user.
     *
     * @param  mixed  $user
     * @return bool
     * @throws \InvalidArgumentException
     */
    public function inactivate($user)
    {
        if (is_string($user) || is_array($user)) {
            $users = $this->getUserRepository();
        ...
        $this->fireEvent('sentinel.inactivating', $user);

        $activations = $this->getActivationRepository();

        $activation = $activations->remove($user);

        $this->fireEvent('sentinel.inactivated', [ $user, $activation ]);
